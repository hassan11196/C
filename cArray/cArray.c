...
void codef(void){
	int codenum;
printf("\n01.Write a C program to read and print elements of array. - using recursion.");
printf("\n02. Write a C program to print all negative elements in an array.");
printf("\n03. Write a C program to find sum of all array elements. - using recursion.");
printf("\n04. Write a C program to find maximum and minimum element in array by recursion");
printf("\n05. Write a C program to find second largest element in an array.");
printf("\n06. Write a C program to count total number of even and odd in an array.");
...
printf("\n09. Write a C program to insert an element in an array.");
...
printf("\n17. Write a C program to put even and odd elements of array in two separate arr");
...
int cArrayv1_scan(int*,int);
int cArrayv1_print(int*,int);
void cArrayv1(){
printf("\n1. Write a C program to read and print elements of array. - using recursion\n");
	int A[10];
	cArrayv1_scan(&A[0],0);
	cArrayv1_print(&A[0],0);
}
int cArrayv1_scan(int* A,int cnt){
	if(cnt==10)
	return ;
	else
	scanf("%d",&A[cnt]);
	cArrayv1_scan(&A[0],++cnt);
}
int cArrayv1_print(int* A,int cnt){
	if(cnt==10)
	return ;
	else
	printf("\n%d",A[cnt]);
	cArrayv1_print(&A[0],++cnt);
}

void cArrayv2_scan(int*,int);
void cArrayv2_print_neg(int*,int);
void cArrayv2(){
printf("\n2. Write a C program to print all negative elements in an array.\n");	
	...
	cArrayv2_print_neg(&A[0],0);
	
}
void cArrayv2_scan(int* A,int cnt){
	if(cnt==10)
	return ;
	else{
	fflush(stdin);
...
void cArrayv2_print_neg(int* A,int cnt){
	if(cnt==10)
	return ;
	else{
		if(A[cnt]<0){
...
int Arrayv3_scan(int*,int,int);
void cArrayv3(){
printf("\n3. Write a C program to find sum of all array elements. - using recursion.\n");
	int arr[5],sum=0;
	sum = Arrayv3_scan(arr,0,sum);
	printf("\nSum of array elements : %d",sum);
}
int Arrayv3_scan(int* arr,int cnt ,int sum){
	if(cnt==5)
	return sum;
	else{
		scanf("%d",&arr[cnt]);
	...
	Arrayv3_scan(arr,++cnt,sum);
}

void cArrayv4_scan(int*,int,int,int);
void cArrayv4(){
printf("\n4. Write a C program to find maximum and minimum element in an array.\n");
	int arr[5];
	cArrayv4_scan(arr,0,0,0);
}
void cArrayv4_scan(int *arr,int cnt,int max,int min){
	if(cnt==5){
		printf("\nMaximum value : %d",max);
		printf("\nMinimum value : %d",min);
		return ;
		...
		min = cnt==0 ? arr[0]:min;
		max = cnt==0 ? arr[0]:max;
		min = arr[cnt] < min ? arr[cnt]:min;
		max = arr[cnt] > max ? arr[cnt]:max;
	}	
	cArrayv4_scan(arr,++cnt,max,min);
}

void cArrayv5_scan(int*,int);
int cArrayv5_sec_largest(int*,int,int,int);
void cArrayv5(){
printf("\n5. Write a C program to find second largest element in an array.\n");
	int arr[5],sec_lar=0,max=0;
	cArrayv5_scan(arr,0);
	sec_lar = cArrayv5_sec_largest(arr,0,max,sec_lar);
	printf("\nSecond largest number in array : %d",sec_lar);
}
void cArrayv5_scan(int* arr,int cnt){
	if(cnt==5)
	return;
	else{
	scanf("%d",&arr[cnt]);
	}
	cArrayv5_scan(arr,++cnt);
}
int cArrayv5_sec_largest(int* arr,int cnt,int max,int max_2){
	if(cnt==5){
		return max_2;
	}
	else{
		max = cnt==0 ? INT_MIN:max;
		max_2 = cnt==0 ? INT_MIN:max_2;
	
		if(arr[cnt]>max){
			max_2=max;
			max=arr[cnt];
		}
		else if(arr[cnt] > max_2 && arr[cnt] < max){
            max_2 = arr[cnt];
        }
	}
	
	cArrayv5_sec_largest(arr,++cnt,max,max_2);
}
